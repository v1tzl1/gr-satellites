...
from satellites import pdu_head_tail_soft


class qa_pdu_head_tail_soft(gr_unittest.TestCase):
    def test_pdu_head_tail_soft(self):
        for mode in range(4):
    ...
    def run_mode(self, mode):
        ...
        num = 10
        sizes = [15, 8]
        test_data = [float(f) for f in range(np.max(sizes))]
        msgs = [test_data[:x] for x in sizes]
        pdus = [pmt.cons(pmt.PMT_NIL, pmt.init_f32vector(len(m), m))
                for m in msgs]
        head_tail = pdu_head_tail_soft(mode, num)
        ...
        for j, msg in enumerate(msgs):
            out = pmt.f32vector_elements(pmt.cdr(dbg.get_message(j)))
            if mode == 0:
                expected = msg[:num]
            elif mode == 1:
                expected = msg[:-num]
...
if __name__ == '__main__':
    gr_unittest.run(qa_pdu_head_tail_soft)
